...
import '../dashboard/nav_bar_screen.dart';
import 'package:six_pos/controller/menu_controller.dart' as mc;
...
class _InVoicePrintScreenState extends State<InVoicePrintScreen> {
  ...
  Future<bool> haha() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (await prefs.get(widget.orderId.toString()) == null) {
      print("init pass");
    } else {
      print("Forcing Redirect...init");
    ...
    BluetoothPrinter device = await ls.retrieveBluetoothPrinter("anik");
    selectDevice(device);
    Get.offAll(ItemsScreen());
  }
  ...
  void selectDevice(BluetoothPrinter device) async {
    ...
    SharedPreferencesService ls = SharedPreferencesService();
    device = await ls.retrieveBluetoothPrinter("anik");

    selectedPrinter = device;
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (await prefs.get(widget.orderId.toString()) == null) {
      await prefs.setString(widget.orderId.toString(), 'true');
      print("First Print");
      _printReceiveTest();
    } else {
      print("Forcing Redirect...");
      Get.offAll(ItemsScreen());
      return;
      // Get.to(ItemsScreen());
    }

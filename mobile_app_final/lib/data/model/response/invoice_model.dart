...
class Invoice {
  ...
  Invoice.fromJson(Map<String, dynamic> json) {
    ...
    _orderAmount = double.parse(json['order_amount']).toDouble();
    if (json['total_tax'] != null) {
      _totalTax = double.parse(json['total_tax']).toDouble();
    } else {
      _totalTax = 0.0;
    }

    if (json['collected_cash'] != null) {
      _collectedCash = double.parse(json['collected_cash']).toDouble();
    } else {
      _collectedCash = 0.0;
    }

    if (json['extra_discount'] != null) {
      _extraDiscount = double.parse(json['extra_discount']).toDouble();
    } else {
      _extraDiscount = 0.0;
    }
    ...
    if (json['coupon_discount_amount'] != null) {
      _couponDiscountAmount =
          double.parse(json['coupon_discount_amount']).toDouble();
    } else {
      _couponDiscountAmount = 0.0;
    }

    _couponDiscountTitle = json['coupon_discount_title'];
    _paymentId = int.parse(json['payment_id']);
...
class Details {
  ...
  Details.fromJson(Map<String, dynamic> json) {
    _id = json['id'];
    _productId = int.parse(json['product_id']);
    _orderId = int.parse(json['order_id']);
    _price = double.parse(json['price']).toDouble();
    _productDetails = json['product_details'];
    if (json['discount_on_product'] != null) {
      _discountOnProduct = double.parse(json['discount_on_product']).toDouble();
    } else {
    ...
    if (json['tax_amount'] != null) {
      _taxAmount = double.parse(json['tax_amount']).toDouble();
    } else {
      _taxAmount = 0.0;
    }
